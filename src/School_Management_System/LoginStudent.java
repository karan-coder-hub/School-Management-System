...
class LoginStudent extends JFrame implements ActionListener {
    ...
    LoginStudent(JFrame welcomeFrame) {
        ...
        ltitle.setBounds(700, 50, 350, 50);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == blogin) {
            try {
                ...
                PreparedStatement pst = con.prepareStatement("select * from Student2 where username='" + username + "' and password='" + password + "'");
        ...
        if (ae.getSource() == bforget) {
            ...
            new ForgetPasswordStudent(welcomeFrame).setVisible(true);
            dispose();
        }
        if (ae.getSource() == bchange) {
            welcomeFrame.setEnabled(false);
            new ChangePasswordStudent(welcomeFrame).setVisible(true);
            dispose();
        }
    }
}
