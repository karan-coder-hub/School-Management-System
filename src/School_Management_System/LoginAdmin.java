...
class LoginAdmin extends JFrame implements ActionListener {
    ...
    LoginAdmin(JFrame welcomeFrame) {
        ...
        //============set Font Label===========
        ltitle = new JLabel("Admin Login Form");
        ...
        show2.setForeground(Color.orange);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == blogin) {
            try {
                ...
                PreparedStatement pst = con.prepareStatement("select * from admin where username='" + username + "' and password='" + password + "'");
        ...
        if (ae.getSource() == bforget) {
            ...
            new ForgetPasswordAdmin(welcomeFrame).setVisible(true);
            dispose();
        }

        if (ae.getSource() == bchange) {
            welcomeFrame.setEnabled(false);
            new ChangePasswordAdmin(welcomeFrame).setVisible(true);
            dispose();
        }
    }
}
